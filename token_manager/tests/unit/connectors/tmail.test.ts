...
describe('TmailConnector', () => {
  ...
  beforeEach(() => {
    connector = new TmailConnector(mockServiceConfig, oidcIssuer)
    mockFetch = vi.fn()
  ...
  it('has serviceId set to twake-mail', () => {
  ...
  describe('refresh', () => {
    it('calls the LemonLDAP token endpoint with grant_type=refresh_token', async () => {
      ...
      const instanceUrl = 'https://jmap.twake.local'
    ...
    it('throws when token endpoint fails', async () => {
      ...
      await expect(
        connector.refresh('bad-token', mockTenant, 'https://jmap.twake.local'),
      ).rejects.toThrow()
    })
  })
